...
for num_node in "${num_node_arr[@]}"; do
    for alpha in "${alphas[@]}"; do
        ...
        while(( "${idx}" <= "${max_idx}" )); do
            ...
            genhrg -n ${num_node} -alpha ${alpha} -deg ${deg} -edge 1 -coord 1 -file ./${save_dir}/${num_node}_${alpha_str}/${idx} -rseed -1 -aseed -1 -sseed -1 -threads 1 > /dev/null
            # get R
