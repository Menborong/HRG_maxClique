...
num_node_arr=(562000 745000 1000000)
...
for num_node in "${num_node_arr[@]}"; do
    for alpha in "${alphas[@]}"; do
        alpha_str=${alpha/./}
        Cid_max=${#degs[@]}
        for ((Cid=0; Cid<Cid_max; Cid++)); do
            deg=${degs[$Cid]}
            valC=${valCs[$Cid]}
            mkdir ./${save_dir}/${num_node}_${alpha_str}_${valC}
            idx=1
            ...
            while(( "${idx}" <= "${max_idx}" )); do
                ...
                genhrg -n ${num_node} -alpha ${alpha} -deg ${deg} -edge 1 -coord 0 -file ./${save_dir}/${num_node}_${alpha_str}_${valC}/${idx} -rseed -1 -aseed -1 -sseed -1 -threads 8 > /dev/null
                # get R
