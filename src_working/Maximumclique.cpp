...
std::vector<int> HRG_CLIQUE::getMaxClique(std::vector<std::vector<int>>& adjs,
                                          std::vector<Node>& geometry, double R,
                                          int N) {
...
// std::vector<int> HRG_CLIQUE::getMaxCliqueRed(
//     std::vector<std::vector<int>> &adjs, std::vector<Node> &geometry, double
//     R, int N) {
//     // initalize solution
...
//             // find common neighbor vertices
//             int tar = (newAdjs[u].size() < newAdjs[v].size())
//                           ? u
//                           : v;  // for speed up, choose the smaller one
//             for (int k : newAdjs[tar]) {
//                 if (k == u || k == v) continue;
//                 if (getDist(geometry[bckId[u]], geometry[bckId[k]]) > d ||
//                     getDist(geometry[bckId[v]], geometry[bckId[k]]) > d)
//                     continue;
//                 vertices.push_back(k);
//             }
...
// std::vector<int> HRG_CLIQUE::getMaxCliqueSkip(
//     std::vector<std::vector<int>> &adjs, std::vector<Node> &geometry, double
//     R, int N) {
//     // initalize solution
//     std::vector<int> init_solution = getMaximalClique(adjs, N);
...
// std::vector<int> HRG_CLIQUE::getMaxCliqueOpt(
//     std::vector<std::vector<int>> &adjs, std::vector<Node> &geometry, double
//     R, int N) {
//     // initalize solution
//     std::vector<int> init_solution = getMaximalClique(adjs, N);
...
//         std::vector<int> candidate = getCobipMaxClique(vertices,
//         cobipartite); candidate.push_back(u); candidate.push_back(v);
...
// std::vector<int> HRG_CLIQUE::getMaxClique(std::vector<std::vector<int>>
// &adjs,
//                                           int N) {
//     // initalize solution
//     std::vector<int> init_solution = getMaximalClique(adjs, N);
...
std::vector<int> HRG_CLIQUE::_getMaxCliqueGeoVer1(
...
std::vector<int> HRG_CLIQUE::_getMaxCliqueGeoVer2(
    const std::vector<std::vector<int>>& adjs, const int N,
    const std::vector<Node>& geometry, const double R) {
    // initalize solution
    std::vector<int> init_solution = getMaximalClique(adjs, N);
    ...
    for (int u = 1; u <= newN; u++) {
        for (int v : newAdjs[u]) {
            ...
            // find common neighbor vertices
            // for speed up, choose the smaller one
            int tar = (newAdjs[u].size() < newAdjs[v].size()) ? u : v;
            for (int k : newAdjs[tar]) {
                if (k == u || k == v) continue;
                if (getDist(geometry[bckId[u]], geometry[bckId[k]]) > d ||
                    getDist(geometry[bckId[v]], geometry[bckId[k]]) > d)
