...
struct MinVertexCover {
    ...
    std::vector<int>
        list;  // list[i] = index of the next edge to be explored from i
    ...
    MinVertexCover(int NX, int NY) : NX(NX), NY(NY) {
        adjs = std::vector<std::vector<int>>(NX);
        XtoY = std::vector<int>(NX, -1);
        YtoX = std::vector<int>(NY, -1);
        level = std::vector<int>(NX);
        list = std::vector<int>(NX);
        VCX = std::vector<int>(NX, 1);
        VCY = std::vector<int>(NY);
    }
